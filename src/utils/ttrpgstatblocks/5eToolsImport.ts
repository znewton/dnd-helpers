...
function stringify(
  property: Record<string, any> | string | any[] | number | boolean | undefined,
  depth: number = 0,
  joiner: string = ' ',
  parens = true,
): string {
...
export function build5eMonsterFromJson(monster: Creature5eTools): ExactPartial<Monster> {
  return {
    image: undefined,
    name: monster.name,
    ...
    subtype: '',
    size: monster.size ? SIZE_ABV_TO_FULL[(monster.size[0] as Size)] : undefined,
    alignment: getMonsterAlignment(monster),
    hp: monster.hp && 'average' in monster.hp
        ? monster.hp?.average
        : undefined,
    hit_dice: monster.hp && 'formula' in monster.hp
        ? monster.hp?.formula
        : '',
...
function getSaves(monster: Creature5eTools): Monster['saves'] {
    ...
    Object.entries(monster.save ?? {}).forEach(
        (
            ...
            if (!v) return;
            entries.push({ [abilityMap[(thr[0] as string | number)] as string]: v });
        });
    return entries;
}
...
function extractSpellsBlocks(spellBlock: EntrySpellcasting): ExtractedSpells {
  ...
  if ('spells' in spellBlock) {
    try {
      for (const level in Object.keys(spellBlock.spells ?? {})) {
        const block = spellBlock.spells[level as keyof typeof spellBlock.spells];
        if (!block) continue;
        const { spells } = block;
        let name: string = `${
          (spellMap as Record<string, string>)[level as keyof typeof spellBlock.spells]
        }`;
  ...
  for (const frequency of frequencyCasting) {
    if (frequency in spellBlock) {
      ...
      const entries = getSpellsFromFrequency(spellBlock[frequency] as EntrySpellcastingFrequency);
...
function getAlignmentString(alignment: Align[] | Align | Alignment): string | undefined {
...
function normalizeEntries(trait: {
  ...
  const flattenedEntries = (trait.entries ?? []).reduce<{ name: string; entries: Entry[]; }[]>(
    (acc, current) => {
