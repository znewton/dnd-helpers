...
function buildFilterableDataTable<T>(
  ...
  columns.forEach(({ name, filterType }) => {
    ...
    if (filterType === 'equals') {
      filterConditions.push(`WHERE ${frontMatterId} = "" OR ${nameStr} = ${frontMatterId}`);
    } else if (filterType === 'contains') {
      filterConditions.push(`WHERE icontains(${nameStr}, ${frontMatterId}) OR icontains(${frontMatterId}, ${nameStr})`);
    }
  });
...
export async function generateDataTableFiles() {
  const outputDir = path.join(config.outputRootDir ?? '/', config.outputDirs.datatables);
