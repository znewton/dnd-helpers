...
function formatItemType(item: IItem) {
	...
	const footnote =
		item.typeEntries.length > 0
			? `[^t-${ItemTypeAbbrev[item.type]}]`
			: undefined;

	if (item.weapon) {
		return `${ItemTypeAbbrev[item.type]}${footnote ?? ''}, (_${
			item.weaponCategory
		}_)`;
	}
...
function formatItemValue(item: IItem) {
	if (item.value === undefined) {
		...
		const isConsumable = ['potion', 'scroll'].includes(
			item.type ?? 'unknown'
		);
		if (isConsumable) {
			return valueByRarityHalved[
				item.rarity as keyof typeof valueByRarity
			];
		}
...
function formatItemProperties(item: IItem) {
	...
	item.property?.forEach((property) => {
		...
		if (propertyDetails) {
			const footnote = propertyDetails.entries?.length
				? `[^p-${property}]`
				: '';
...
function formatItemFootnotes(item: IItem): string {
	...
	const createFootnote = (key: string, entries: Entry[]): string =>
		`[^${key}]: ${entriesToMarkdown(entries).replace(/^(\n)*/, '')}`;
	if (typeDetails.length && item.type) {
		footnotes.push(createFootnote(`t-${item.type}`, typeDetails));
	}
	propertyDetails.forEach(([propertyAbbrev, propertyDetail]) => {
		if (!propertyDetail.entries?.length) return;
		footnotes.push(
			createFootnote(`p-${propertyAbbrev}`, propertyDetail.entries)
		);
	});
...
function itemToMarkdown(item: IItem): string {
	...
	const rarity =
		item.rarity === 'none' ? undefined : toTitleCase(item.rarity);
