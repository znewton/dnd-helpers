...
import { CreatureSize, IRace, listBooks, listRaces } from '../data';
import {
	...
	return race.ability
		.map((abilityScores) =>
			Object.entries(abilityScores).map(
				([abilityName, abilityBonus]) =>
					`${toTitleCase(abilityName)} +${abilityBonus}`
			)
		)
		.join('; ');
}
		...
		.map((languageProf) =>
			Object.entries(languageProf)
				.map(([language, known]) => {
					...
					throw new Error(
						`Unhandled language for race: ${race.name}`
					);
				})
				.join(', ')
		)
...
export async function generateRacesFiles() {
	...
	books
		.sort(
			(bookA, bookB) =>
				new Date(bookA.published).getTime() -
				new Date(bookB.published).getTime()
		)
		.forEach((book, i) => {
	...
	Object.entries(raceSources).forEach(([name, sources]) => {
		const srcs = [...sources].sort((sourceA, sourceB) => {
			const [orderA, orderB] = [
				sourcePublishOrder[sourceA.toLowerCase()],
				sourcePublishOrder[sourceB.toLowerCase()],
			];
			...
			return (
				(orderB ?? Number.MIN_SAFE_INTEGER) -
	...
	races.forEach((race) => {
		if (
			...
			(raceSources[race.name] ?? [])[0]?.toLowerCase()
		) {
		...
		const filePath = path.join(
			outputDir,
			`${normalizeFilename(race.name)}.md`
		);
