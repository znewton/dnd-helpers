...
export interface ISpellComponents {
	...
	m?:
		| string
		| { text: string; cost: number; consume?: boolean | undefined }
		| undefined;
}
...
export async function listSpells(
	...
	[...spellFiles].forEach((filename) => {
		spellReadPs.push(
			getJsonData<SpellJson>(filename, spellsBaseUrl, { spell: [] })
		);
	});
	const spellFluffReadPs: Promise<SpellFluffJson>[] = [];
	if (options.includeFluff) {
		const spellFluffFiles = ownedSourceBooks.map(
			(alias) => `fluff-spells-${alias}.json`
		);
		[...spellFluffFiles].forEach((filename) => {
			spellFluffReadPs.push(
				getJsonData<SpellFluffJson>(filename, spellsBaseUrl, {
					spellFluff: [],
				})
			);
		});
	...
	spellJsons.forEach(({ spell }) => {
		spell.forEach((spellDetails) => {
			spells.push({
				...
				classes: Array.from(
					spellClassMap[spellDetails.name.toLowerCase()] ?? []
				),
			});
		});
	});
