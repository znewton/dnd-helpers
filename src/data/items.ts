...
export interface IItemBase {
   ...
   */
  packContents?: ({ item: string; quantity: number; } | string | { special: string; })[] | undefined;
}
...
export interface IItemEx extends IItemBase {
  ...
  ability?: IModifyAbility | AbilityModifier | AbilityModifierChoice | undefined;
 ...
 */
export interface IItem extends Pick<IItemEx,
| 'name'
| 'source' | 'page' | 'otherSources'
...
| 'reqAttune' | 'strength'
| 'stealth' | 'ac'
| 'range' | 'dmg1' | 'dmgType' | 'dmg2' > {
  entries: Entry[];
  propertyDetails: Record<string, { fullName: string; entries?: Entry[] | undefined; }>;
  typeEntries: Entry[];
}
...
function replaceItemPropTemplate(item: IItemEx, template: string): string {
  return template.replace(/\{\{item\.([a-zA-Z0-9]+)\}\}/g, (_match, prop) => {
    ...
    if (!itemProp) {
      throw new Error(`Item Property, ${prop}, for item entry not found for ${item.name}`);
    }
    return `${itemProp}`;
  });
}

function convertItemDataToItem(data: IItemEx, refs: IConversionReferenceData): IItem {
  const item: IItem = {
    name: data.name,
    source: data.source,
    page: data.page,
    otherSources: [...(data.otherSources ?? []), ...(data.additionalSources ?? [])],
    type: data.type,
    weapon: data.weapon,
  ...
  if (data.hasRefs) {
    ...
    const stringifiedReplacedEntries = JSON.stringify(rawEntries)
      .replace(/\{#itemEntry ([a-zA-Z0-9 -']+)\|?[a-zA-Z0-9 -']*\}/g, (match, p1) => {
        const itemEntry = refs.itemEntryMap[p1];
        if (!itemEntry) {
          throw new Error(`Item Entry for ${data.name}'s ${match} Not Found`);
        }
  ...
  item.property?.forEach((propertyAbbrev) => {
    ...
    item.propertyDetails[propertyAbbrev] = {
      fullName: itemProperty.template
        ? replaceItemPropTemplate(data, itemProperty.template)
          .replace('{{prop_name}}', propName)
        : propName,
      entries: itemProperty.entries,
    };
  });
  if (item.type && (refs.itemTypeMap[item.type] || refs.itemTypeAdditionalEntriesMap[item.type])) {
  ...
  if (data._copy) {
    ...
    if (!itemDataToCopy) {
      throw new Error(`Copy Item (${data._copy.name}) Not Found for ${data.name}`);
    }
    const copiedItem = { ...convertItemDataToItem(itemDataToCopy, refs) };
    const preservedElements: any = {};
    Object.entries(data._copy._preserve ?? {}).forEach(([key, preserve]) => {
      if (!preserve) return;
...
export async function listItems(): Promise<IItem[]> {
  const itemsBaseJson = await getJsonData<ItemsBaseJson>(
    ...
    itemsBaseBaseUrl,
    {
      baseitem: [], itemProperty: [], itemType: [], itemEntry: [], itemTypeAdditionalEntries: [],
    },
  );
  ...
  itemsBaseJson.itemProperty.forEach((itemProperty) => {
    conversionRefs.itemPropertyMap[itemProperty.abbreviation] = itemProperty;
  });
  ...
  itemsBaseJson.itemTypeAdditionalEntries.forEach((itemTypeAdditionalEntries) => {
    conversionRefs.itemTypeAdditionalEntriesMap[itemTypeAdditionalEntries.appliesTo] = itemTypeAdditionalEntries;
  });
